	...
	for (int i = 1; i <= m; i++) {
		for (int j = 1; j <= n + i - 1; j++) {
			if (i == m) {
				...
				g2.addedge(id[i][j] * 2, t, n, 0);
				g3.addedge(id[i][j] * 2, t, n, 0);
			} else {
				...
				g3.addedge(id[i][j] * 2, id[i + 1][j] * 2 - 1, n, 0);
				g3.addedge(id[i][j] * 2, id[i + 1][j + 1] * 2 - 1, n, 0);
			}
		}
	}
	for (int i = 1; i <= m; i++) {
		for (int j = 1; j <= n + i - 1; j++) {
			...
			g2.addedge(id[i][j] * 2 - 1, id[i][j] * 2, n, -x);
			g3.addedge(id[i][j] * 2 - 1, id[i][j] * 2, n, -x);
		}
	}
	auto [flow1, cost1] = g1.solve();
	auto [flow2, cost2] = g2.solve();
	auto [flow3, cost3] = g3.solve();
	printf("%lld\n%lld\n%lld\n", -cost1, -cost2, -cost3);
}